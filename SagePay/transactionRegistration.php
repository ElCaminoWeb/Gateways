		...
		** Registration results come back in the Status and StatusDetail fields */
		...
		{ 
            ...
            ** after the customer is redirected to the PayPalRedirectURL to go through PayPal authentication */
			...
			ob_end_flush();
			//echo $strPayPalRedirectURL;
			$temp = explode("=",$strPayPalRedirectURL);
			$token = $temp[1];
			//print_r($temp);
			$url = "https://www.sandbox.paypal.com/webscr?cmd=_express-checkout&token=" . $token;	
			echo "<a href=\"" . $url . "\"> Token: ". $token . "</a>" ; 
			//redirect($url);
			//exit();
		}
		else
...
<body>
    <div id="pageContainer">
		...
		if ($strPageState=="Payment") 
		{
			...
			<table class="formTable">
				...
				<tr> 
					...
					<td class="fieldData"><input name="CardHolder" type="text" value="<?php echo $strCardHolder ?>" size="25" maxlength="50"></td>
				</tr>
				<tr> 
					<td class="fieldLabel">Card Number:</td>
			  		<td class="fieldData"><input name="CardNumber" type="text" value="<?php echo $strCardNumber ?>" size="25" maxlength="24">
					&nbsp;<font size="1">(With no spaces or separators)</font></td>
				</tr>
				<tr> 
					<td class="fieldLabel">Start Date:</td>
			  		<td class="fieldData"><input name="StartDate" type="text" value="<?php echo $strStartDate ?>" size="5" maxlength="4">
				   	&nbsp;<font size="1">(Where available. Use MMYY format  e.g. 0207)</font></td>
				</tr>
				<tr> 
					<td class="fieldLabel">Expiry Date:</td>
			  		<td class="fieldData"><input name="ExpiryDate" type="text" value="<?php echo $strExpiryDate ?>" size="5" maxlength="4">
				   	&nbsp;<font size="1">(Use MMYY format with no / or - separators e.g. 1109)</font></td>
				</tr>
				<tr> 
					<td class="fieldLabel">Issue Number:</td>
			  		<td class="fieldData"><input name="IssueNumber" type="text" value="<?php echo $strIssueNumber ?>" size="5" maxlength="2">
		...
		else // The customer has already entered their card details and we're displaying the result
		{
			...
			<input type="hidden" name="navigate" value="" />
			<input type="hidden" name="CompletionURL" value="<?php echo $strCompletionURL ?>">
